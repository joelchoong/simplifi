...
const EDITOR_DRAFT_STORAGE_KEY = "financial-records-editor-draft";
...
export function NetWorthRecordsEditor({
  ...
  const [draftRecords, setDraftRecords] = useState<NetWorthRecord[]>(records);
  ...
  useEffect(() => {
    const storedDraft = window.sessionStorage.getItem(EDITOR_DRAFT_STORAGE_KEY);
    if (!storedDraft) {
    ...
    try {
      const parsed = JSON.parse(storedDraft) as {
        draftRecords?: NetWorthRecord[];
        deletedRecordIds?: string[];
        newMonth?: string;
      };

      setDraftRecords(parsed.draftRecords ?? records);
      setDeletedRecordIds(parsed.deletedRecordIds ?? []);
      setNewMonth(parsed.newMonth ?? getMonthStart().slice(0, 7));
    } catch {
      setDraftRecords(records);
      setDeletedRecordIds([]);
    }
  ...
  }, [deletedRecordIds, draftRecords, newMonth]);

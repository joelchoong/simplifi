...
import { formatCurrency } from "@/features/money-health/retirement/domain/epfCalculations";
...
const EPFChart: React.FC<EPFChartProps> = ({ data, retirementAge = 60 }) => {
  ...
  const sampledData = useMemo(() => {
    ...
    return data.filter((d, i) => i === 0 || i === data.length - 1 || d.age % 5 === 0);
  }, [data]);
  ...
  const xForIndex = (i: number) => margin.left + (innerWidth * i) / Math.max(1, sampledData.length - 1);

  const yForValue = (v: number) => margin.top + innerHeight - (innerHeight * v) / Math.max(1, maxAmount);
    ...
    <div className="w-full" ref={containerRef}>
      <div className="relative overflow-hidden">
        <svg width={width} height={chartHeight + margin.top + margin.bottom}>
            ...
            return (
              <g key={i}>
                <line
                  ...
                  strokeDasharray="4,4"
                />
                <text x={margin.left - 12} y={y + 4} textAnchor="end" fontSize="12" fill="#4b5563">
          ...
          >
            {width < 640 ? "Total Amount" : "Total EPF Amount"}
          </text>
        </svg>
      </div>
    </div>
